    ...
    private void sendMessage(User user) {
        if (!StringUtils.isEmpty(user.getEmail())) {
            String message = String.format(
                    "Привет, %s \n" +
                            "Добро пожаловать в Sweater. " +
                            "Для подтверждения адреса электронной почты перейдите по <a target=\"_blank\" href=\"" +
                            link + "/activate/%s" + "\">ссылке</a>.",
                    user.getUsername(),
